...
const WelcomeModal: React.FC<WelcomeModalProps> = ({ onClose }) => {
  ...
  const renderContent = () => {
    switch (step) {
      case 1:
        return (
          <div className="space-y-8">
            <span className="text-[10px] font-black tracking-[0.8em] text-[#FF007F] uppercase block animate-pulse">Introduction</span>
            <h2 className="text-5xl md:text-7xl serif text-black leading-tight uppercase tracking-tighter">
              Welcome <br/><span className="italic text-[#FF007F]">Atelier.</span>
            </h2>
            <p className="text-black/70 text-base md:text-lg leading-relaxed font-light max-w-2xl">
      ...
      case 2:
        return (
          <div className="space-y-8">
            <span className="text-[10px] font-black tracking-[0.8em] text-[#FF007F] uppercase block">Opportunity</span>
            <h2 className="text-5xl md:text-7xl serif text-black leading-tight uppercase tracking-tighter">
              The <span className="italic">Vision.</span>
            </h2>
            <div className="bg-neutral-50 p-8 md:p-12 border-l-4 border-[#FF007F] space-y-6 shadow-sm">
              <p className="text-base md:text-lg text-black/70">
                Mitchel Mhizha is opening a limited investment opportunity for one capital partner at <span className="text-black font-bold text-xl">$500</span>. This is a 1:1 matched investment:
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-[11px] font-black tracking-[0.2em] uppercase text-black">
                <div className="bg-white p-6 border border-black/5 shadow-sm">
                  <span className="opacity-30 block mb-2">Mitchel's Part</span>
                  <span className="text-xl">$500 (Secured)</span>
                </div>
                <div className="bg-white p-6 border border-black/5 shadow-sm">
                  <span className="opacity-30 block mb-2">Partner Part</span>
                  <span className="text-xl text-[#FF007F]">$500</span>
                </div>
              </div>
              <div className="pt-4 border-t border-black/5">
                ...
                <p className="text-[10px] text-black/40 mt-1 uppercase font-bold">Directly allocated to resale inventory</p>
              </div>
            </div>
          </div>
        );
      case 3:
        return (
          <div className="space-y-8">
            <span className="text-[10px] font-black tracking-[0.8em] text-[#FF007F] uppercase block">Growth</span>
            <h2 className="text-5xl md:text-7xl serif text-black leading-tight uppercase tracking-tighter">
      ...
      case 4:
        return (
          <div className="space-y-8">
            <span className="text-[10px] font-black tracking-[0.8em] text-[#FF007F] uppercase block">Participation</span>
            <h2 className="text-5xl md:text-7xl serif text-black leading-tight uppercase tracking-tighter">
  ...
  return (
    <div 
      ref={overlayRef}
      className="fixed inset-0 z-[2000] bg-white flex items-center justify-center p-4 md:p-12 opacity-0 overflow-hidden"
    >
      <div className="absolute top-0 left-0 w-full h-full pointer-events-none overflow-hidden select-none flex items-center justify-center">
        <h1 
          ref={watermarkRef}
          className="text-[35vw] font-black leading-none uppercase serif text-black opacity-0 -translate-x-10"
        >
          {getWatermark()}
        </h1>
      </div>
      ...
      <div 
        ref={contentRef}
        className="relative z-10 max-w-5xl w-full bg-white border border-black/5 shadow-[0_60px_120px_rgba(0,0,0,0.1)] p-10 md:p-24 overflow-hidden flex flex-col min-h-[600px] justify-between"
      >
        <div ref={innerContentRef} className="flex-grow">
          {renderContent()}
        </div>

        <div className="mt-16 flex flex-col md:flex-row items-center justify-between gap-10 border-t border-black/10 pt-16">
          <div className="flex gap-4">
            {[1, 2, 3, 4].map((s) => (
              <div 
                key={s} 
                className={`h-[1px] transition-all duration-700 ${s === step ? 'w-16 bg-[#FF007F]' : 'w-8 bg-black/10'}`}
              />
            ))}
          </div>
          ...
          <button 
            onClick={handleNext}
            className="group relative overflow-hidden w-full md:w-auto px-20 py-6 border-2 border-black rounded-full transition-all duration-500 hover:border-[#FF007F]"
          >
            ...
            <span className="relative z-10 text-[11px] font-black tracking-[0.6em] uppercase text-black group-hover:text-white transition-colors duration-500">
